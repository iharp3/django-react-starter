...
class FindTimeExecutor(QueryExecutor):
    ...
    def _execute_pyramid_hour(self):
        ...
        result = xr.Dataset(
            data_vars={self.variable_short_name: ([TIME_DIMENSION_LIT], [None] * len(time_points))},
            coords=dict(TIME_DIMENSION_LIT=time_points),
        )
        ...
        if years:
            ...
            for year in years:
                ...
                curr_year_min = year_min[self.variable_short_name].sel(TIME_DIMENSION_LIT=year_datetime).values.item()
                curr_year_max = year_max[self.variable_short_name].sel(TIME_DIMENSION_LIT=year_datetime).values.item()
        ...
        if months:
            ...
            for month in months:
                ...
                curr_month_min = month_min[self.variable_short_name].sel(TIME_DIMENSION_LIT=month_datetime).values.item()
                curr_month_max = month_max[self.variable_short_name].sel(TIME_DIMENSION_LIT=month_datetime).values.item()
                if self.filter_predicate == ">":
        ...
        if days:
            ...
            for day in days:
                ...
                curr_day_min = day_min[self.variable_short_name].sel(TIME_DIMENSION_LIT=day_datetime).values.item()
                curr_day_max = day_max[self.variable_short_name].sel(TIME_DIMENSION_LIT=day_datetime).values.item()
                if self.filter_predicate == ">":
        ...
        result_undetermined = result[TIME_DIMENSION_LIT].where(result[self.variable_short_name].isnull(), drop=True)
